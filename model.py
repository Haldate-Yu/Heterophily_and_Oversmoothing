...
class GPRGNN(torch.nn.Module):
    def __init__(self, nfeat, nlayers, nhidden, nclass, dropout, dprate_GPRGNN, alpha_GPRGNN, Gamma_GPRGNN, Init_GPRGNN,
                 ppnp_GPRGNN):
...
class GAT(nn.Module):
    def __init__(self, nfeat, nhid, nlayers, nclass, dropout, alpha, nheads, use_sparse=False):
        ...
        for j in range(nlayers - 2):
            attentions = [model_sel(nhid * nheads, nhid, dropout=dropout, alpha=alpha, concat=True) for _ in
                          range(nheads)]
            for i, attention in enumerate(attentions):
